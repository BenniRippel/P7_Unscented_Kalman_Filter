 ...
 ******************************************************************************/
    ...
    for (int i = 0; i < n_aug; i++) {
        Xsig_aug.col(i + 1) = x_aug + sqrt(lambda + n_aug) * Sqrt_P_aug.col(i);
        Xsig_aug.col(i + 1 + n_aug) = x_aug - sqrt(lambda + n_aug) * Sqrt_P_aug.col(i);
    }
 ...
 ******************************************************************************/
    ...
    for (int i=0; i<2*n_aug+1; i++)
    {
        VectorXd vec = Xsig_pred.col(i)-x;
        //angle normalization
        while (vec(3)> M_PI) vec(3)-=2.*M_PI;
        while (vec(3)<-M_PI) vec(3)+=2.*M_PI;

        P = P+ weights(i)*vec*vec.transpose();
    }
 ...
 ******************************************************************************/
    ...
    for (int i=0; i<2*n_aug+1; i++)
    {
        ...
        while (vec(1)<-M_PI) vec(1)+=2.*M_PI;
        S = S + weights(i)*vec*vec.transpose();
    }
    S=S+R;
 ...
 ******************************************************************************/
    ...
    for(int i=0; i<2*n_aug+1; i++)
    {
        VectorXd x_vec = Xsig_pred.col(i)-x;
        ...
        Tc = Tc + weights(i)*x_vec*z_vec.transpose();
    }
